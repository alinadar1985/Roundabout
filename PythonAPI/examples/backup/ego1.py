...
def main():
    ...
    try:
        ...
        while True:
            ...
            '''
            #waypoints = carla_map.get_waypoint(ego_vehicle.get_location(),project_to_road=True, lane_type=(carla.LaneType.Driving))
            if point == None:
                nexts = waypoint.next(4.0)
            else:
                nexts = point.next(4.0)
            ...
            if len(queue) < 2: 
                ...
                ego_vehicle.set_transform(point.transform)
                
            else:
                A = queue[1].transform.location
                B = queue[0].transform.location

                if len(nexts) > 1:
                    location1 = nexts[0].transform.location
                    position1 = (B.x - A.x) * (location1.y - A.y) - (B.y - A.y) * (location1.x - A.x)
                    print(str(position1))
                    print(str(location1))
                    location2 = nexts[1].transform.location
                    position2 = (B.x - A.x) * (location2.y - A.y) - (B.y - A.y) * (location2.x - A.x)
                    print(str(position2)) 
                    print(str(location2)) 
