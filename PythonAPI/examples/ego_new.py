...
def main():
    ...
    try:
        ...
        def rad_callback(radar_data, side):
            #print("Detected objects ("+ side +"):" + str(len(radar_data)))
            objects = []
            rad_counter = 1
            ...
            for detect in radar_data:
                ...
                detected.distance = round(detect.depth, 2)
                detected.x_distance =  round(detect.depth * math.cos(azi * math.pi / 180) , 2)
                detected.y_distance = round(detect.depth *  math.sin(azi * math.pi / 180), 2)
                detected.z_distance = round(detect.depth *  math.sin(alt * math.pi / 180), 2)
                detected.speed = round(detect.velocity, 2)
                detected.x_speed = round(detect.velocity * math.cos(azi * math.pi / 180), 2)
                detected.y_speed = round(detect.velocity * math.sin(azi * math.pi / 180) , 2)    
                if detected.z_distance > (-0.8) + 0.1 : # -0.8 is the elevation of surface relatively to sensor, 0.1 is the surface threshold
            ...
            while len(not_shaped) > 0:
                ...
                for i in range(1, len(not_shaped)):
                    ...
                    ######################################  find the left object ################################################
                        ...
                        if distance < 1.2 and abs(not_shaped[i].speed - root.speed) < 1.0 :
            ...
            if side == "Front":
                world.debug.draw_string(carla.Location(4,-15,0), side + " : " + str(len(shapes)) + " vehs.", False,  carla.Color(0,0,0,0), 1) 
        ...
        rad_front_bp.set_attribute('range', str(8)) # 8 meters sensor coverage
        rad_front_bp.set_attribute("sensor_tick",str(2.0)) # initialize the tick interval in 2 seconds
        rad_front_location = carla.Location(x=1.8, z=0.8) # set the location of sensor with 1.8 m in X-axis and 0.8 m in Z-axis
        rad_front_rotation = carla.Rotation(pitch=5) # rotate sensor 5 degrees in Y-axis
        ...
        rad_back_bp.set_attribute('range', str(8)) # 8 meters sensor coverage
        rad_back_bp.set_attribute("sensor_tick",str(2.0))  # initialize the tick interval in 2 seconds
        rad_back_location = carla.Location(x=-1.8, z=0.8) # set the location of sensor with -1.8 m in X-axis and 0.8 m in Z-axis
        rad_back_rotation = carla.Rotation(pitch=170) # rotate sensor 170 degrees in Y-axis
